...
class Dungeon:
    ...
    @staticmethod
    def encounter(x: int, y: int) -> MonsterMeta:
        """
        ...
        if monsters:
            ...
            if not dungeon[new_x][new_y]:
                ...
                for clt_name, clt in Client.client_list.items():
                    msg = (f"{monster_name} moved one cell {move}")
        ...
        match msg:
            ...
            case ["addmon", *args]:
                ...
                *tmp, replace = Dungeon.add_monster(new_monster)
                ...
                for name, clt in Client.client_list.items():
                    if clt is client:
                        ans = (f"Added monster {monster_name} to {monster_coords} saying {monster_msg}")
                        writer.write(ans.encode())
                    else:
                        ans = (f"{client.name} added monster {monster_name} to {monster_coords} saying {monster_msg}")
                    ...
                    for clt_name, clt in Client.client_list.items():
                        if clt is client:
                            ...
                            if not data["died"]:
                                ...
                                ans = ("{} died.").format(data["attack_name"])
                            writer.write(ans.encode())
                            await writer.drain()
                        else:
                            clt.locale.install()
